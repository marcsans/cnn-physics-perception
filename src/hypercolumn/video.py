...
def find_center_ball(m, threshold, shape):
    ...
    resize_ratio = 1.0 * shape[1] / shape[0]
    cx *= surface_ratio
    cy *= surface_ratio * resize_ratio
    return cx, cy
...
#     centers.append(find_center_ball(m, threshold, shape))
#     if write_video:
...
centers = [(c[0],852/480*c[1]) for c in centers]
radius = find_radius(centers)
point = find_attach_point(radius)
print point[0]/480
print point[1]/852
angles = [np.arctan2((c[0]-point[0]),(c[1]-point[1])) for c in centers]

np.savetxt('angle_sequence.npy', angles)
#print angles